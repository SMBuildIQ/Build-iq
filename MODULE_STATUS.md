...
| 29 | Automated testing | FULL | Unit/integration coverage (tenant isolation, permissions, policy engine, AI extraction, invoice matching, notifications, quote document extraction, price benchmarking, org-admin CRUD guards) plus real Playwright/Chromium end-to-end tests (`tests/e2e/`) driving both the Flow 1 create-purchase-request journey and a UI-level cross-tenant access check, and now Flow 2 — RFQ creation → send → an unauthenticated supplier submitting a real quote through the bare-token portal in a separate browser context → compare → select → policy-engine-required approval → PO issuance — through an actual browser end to end, wired into CI as a separate job |
