...
| 17 | Human-assisted negotiation | FULL | AI drafts, human sends — no autonomous path exists in code. Sending now dispatches real email via Resend (same integration as RFQ send), with an honest log fallback when no API key or supplier contact is configured. `tests/negotiation-draft.test.ts` (7 tests) covers the deterministic target-price math (floored at 3%, capped at 15% of the gap to the lowest qualified quote) and freight-inclusion logic precisely |
