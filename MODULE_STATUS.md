...
| 26 | Document library | FULL | Upload/download/list, tenant- and entity-ownership-checked; dual-driver storage (local disk default, S3-compatible via `STORAGE_DRIVER=s3`) — S3 driver verified against a real in-process S3-API server, not deployed to a real bucket (see KNOWN_LIMITATIONS.md) |
